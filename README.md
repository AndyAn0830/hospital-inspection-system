...
<body>
    ...
    <div id="mainSystem" class="hidden">
        <div class="container">
            ...
            <div class="tabs">
                ...
                <button class="tab" data-tab="settings">雲端設定</button>
            </div>
            ...
            <div id="settings" class="tab-content">
                <div class="form-section">
                    <h2>☁️ Google Sheets 雲端同步設定</h2>
                    
                    <div class="info-box green" style="margin-bottom: 20px;">
                        <strong>🎉 自動綁定功能：</strong><br>
                        分享系統連結時，可以在網址後加上 <code>?sheet=您的Google_Apps_Script網址</code><br>
                        其他人開啟連結就會自動連結到雲端，不需要手動設定！<br><br>
                        <strong>範例：</strong><br>
                        <code>系統網址.html?sheet=https://script.google.com/macros/s/xxxxx/exec</code>
                    </div>
                    
                    <div class="info-box blue" style="margin-bottom: 20px;">
                        <strong>📌 功能說明：</strong><br>
                        啟用此功能後，所有巡檢記錄會自動同步到 Google 試算表，讓不同裝置都能看到所有記錄。
                    </div>

                    <div class="form-group">
                        <label for="googleSheetsUrl">Google Apps Script 網址</label>
                        <input type="text" id="googleSheetsUrl" placeholder="https://script.google.com/macros/s/...../exec" 
                               value="">
                    ...
                    <div style="margin-top: 20px;">
                        ...
                        <button class="btn btn-success" onclick="saveGoogleSettings()" style="margin-left: 10px;">
                            💾 儲存設定
                        </button>
                    </div>
                    ...
                    <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #e9ecef;">
                        ...
                        <div class="form-group">
                            <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                                ...
                                <span style="font-weight: normal;">啟用自動同步（提交記錄時自動上傳到 Google Sheets）</span>
                            </label>
                        </div>

                        <div class="form-group">
                            <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                                <input type="checkbox" id="autoLoadEnabled" onchange="toggleAutoLoad()" style="width: auto;">
                                <span style="font-weight: normal;">啟用自動載入（開啟系統時自動從 Google Sheets 載入記錄）</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div style="margin-top: 30px;">
                        ...
                        <div style="margin-top: 15px;">
                            <button class="btn btn-primary" onclick="uploadToGoogleSheets()">
                                ⬆️ 上傳所有記錄到 Google Sheets
                            </button>
                            <button class="btn btn-secondary" onclick="downloadFromGoogleSheets()" style="margin-left: 10px;">
                                ⬇️ 從 Google Sheets 載入記錄
                            </button>
                        </div>
                        <div class="info-box" style="margin-top: 15px;">
                            <strong>注意：</strong>
                            <ul style="margin-left: 20px; margin-top: 10px;">
                                <li>「上傳」會將本機所有記錄傳送到 Google Sheets</li>
                                <li>「載入」會將 Google Sheets 的記錄合併到本機</li>
                                <li>系統會自動避免重複記錄</li>
                            </ul>
                        </div>
                    </div>
    ...
    <script>
        ...
        function init() {
            ...
            // 檢查URL參數，自動綁定Google Sheets
            try {
                var urlParams = new URLSearchParams(window.location.search);
                var sheetUrl = urlParams.get('sheet');
                if (sheetUrl) {
                    // 解碼URL（如果有編碼）
                    sheetUrl = decodeURIComponent(sheetUrl);
                    // 儲存到localStorage
                    localStorage.setItem('googleSheetsUrl', sheetUrl);
                    console.log('已自動綁定 Google Sheets:', sheetUrl);
                    showMessage('✓ 已自動連結雲端資料庫', 'success');
                }
            } catch (e) {
                console.log('URL參數解析失敗:', e);
            }
            ...
            function updateCurrentTime() {
                var now = new Date();
                // 轉換為台灣時間（UTC+8）
                var taiwanTime = new Date(now.getTime() + (8 * 60 * 60 * 1000) - (now.getTimezoneOffset() * 60 * 1000));
                document.getElementById('inspectionDateTime').value = taiwanTime.toISOString().slice(0, 16);
            }
        ...
        function addRecord() {
            ...
            if (isWard) {
                ...
                for (var i = 0; i < atpItems.length; i++) {
                    var itemName = atpItems[i];
                    var itemId = recordCounter + '_atp_' + (i + 1);
                    
                    html += '<div class="atp-item">';
                    html += '<label class="atp-item-label">' + itemName + '</label>';
                    ...
                    html += '</div>';
                    html += '<div class="radio-option na">';
                    html += '<input type="radio" id="' + itemId + '_na" name="atp_' + recordCounter + '_' + (i + 1) + '" value="無須檢測">';
                    html += '<label for="' + itemId + '_na">− 無須檢測</label>';
                    html += '</div>';
                    html += '</div>';
                    html += '</div>';
                }
        ...
        function submitInspection() {
            ...
            for (var i = 0; i < records.length; i++) {
                ...
                if (area === '公共區域' && (!item || !item.value.trim())) {
                ...
                var itemData = {
                    ...
                    item: (item && item.value) ? item.value : 'ATP檢測為主',
                    notes: record.querySelector('[name="notes"]') ? record.querySelector('[name="notes"]').value : '',
                    photos: photos,
        ...
        function loadGoogleSettings() {
            try {
                var url = localStorage.getItem('googleSheetsUrl');
                var autoSync = localStorage.getItem('autoSyncEnabled');
                var autoLoad = localStorage.getItem('autoLoadEnabled');
                ...
                if (autoLoad === 'true') {
                    document.getElementById('autoLoadEnabled').checked = true;
                    // 如果啟用自動載入，在初始化時載入資料
                    setTimeout(downloadFromGoogleSheets, 1000);
                }
            } catch (e) {
                console.log('載入Google設定失敗:', e);
            }
        }
        
        // 儲存 Google Sheets 設定
        function saveGoogleSettings() {
            var url = document.getElementById('googleSheetsUrl').value.trim();
            
            if (!url) {
                showMessage('請輸入 Google Apps Script 網址', 'error');
                return;
            }
        ...
        function testGoogleConnection() {
            ...
            if (!url) {
                showMessage('請先輸入 Google Apps Script 網址', 'error');
                return;
            }
            ...
            .then(function() {
                showMessage('連線測試成功！', 'success');
                var statusDiv = document.getElementById('connectionStatus');
                var statusTitle = document.getElementById('statusTitle');
                var statusMessage = document.getElementById('statusMessage');
                
                statusDiv.style.display = 'block';
                statusTitle.textContent = '✓ 連線成功';
                statusTitle.style.color = '#155724';
                statusMessage.textContent = 'Google Sheets 連線正常，可以開始使用雲端同步功能。';
            })
            .catch(function(error) {
                ...
                statusMessage.textContent = '無法連線到 Google Sheets，請檢查網址是否正確。';
            });
        }
        ...
        function toggleAutoSync() {
            ...
            showMessage(enabled ? '已啟用自動同步' : '已關閉自動同步', 'info');
        }
        
        // 切換自動載入
        function toggleAutoLoad() {
            var enabled = document.getElementById('autoLoadEnabled').checked;
            localStorage.setItem('autoLoadEnabled', enabled);
            showMessage(enabled ? '已啟用自動載入' : '已關閉自動載入', 'info');
        }
        ...
        function uploadToGoogleSheets() {
            ...
            if (!url) {
                showMessage('請先設定 Google Apps Script 網址', 'error');
                switchTab('settings');
                return;
            }
            
            if (inspectionRecords.length === 0) {
                showMessage('沒有記錄可以上傳', 'error');
                return;
            }
            
            showMessage('正在上傳 ' + inspectionRecords.length + ' 筆記錄...', 'info');
            ...
            .then(function() {
                showMessage('✓ 成功上傳 ' + inspectionRecords.length + ' 筆記錄到 Google Sheets', 'success');
            })
            .catch(function(error) {
        ...
        function downloadFromGoogleSheets() {
            ...
            if (!url) {
                console.log('未設定 Google Sheets 網址');
                return;
            }
            
            showMessage('正在從 Google Sheets 載入記錄...', 'info');
            
            fetch(url + '?action=download')
            ...
            .then(function(data) {
                if (data && data.records && data.records.length > 0) {
                    // 合併記錄，避免重複
                    var existingIds = inspectionRecords.map(function(r) { return r.id; });
                    var newRecords = data.records.filter(function(r) {
                        return existingIds.indexOf(r.id) === -1;
                    });
                    
                    if (newRecords.length > 0) {
                        inspectionRecords = inspectionRecords.concat(newRecords);
                        // 依日期排序
                        inspectionRecords.sort(function(a, b) {
                        ...
                        showMessage('✓ 從 Google Sheets 載入了 ' + newRecords.length + ' 筆新記錄', 'success');
                        ...
                        showMessage('Google Sheets 沒有新記錄', 'info');
                    }
                } else {
                    showMessage('Google Sheets 目前沒有記錄', 'info');
                }
            })
            .catch(function(error) {
                console.error('載入失敗:', error);
                // 靜默失敗，不顯示錯誤訊息（可能只是網路問題）
            });
        }
        
        // 修改提交記錄函數，加入自動同步
        var originalSubmitInspection = submitInspection;
        submitInspection = function() {
            // 呼叫原始提交函數
            originalSubmitInspection();
            
            // 如果啟用自動同步，上傳到 Google Sheets
            var autoSync = localStorage.getItem('autoSyncEnabled');
            if (autoSync === 'true') {
                setTimeout(function() {
        ...
        // 初始化時載入 Google 設定
        document.addEventListener('DOMContentLoaded', function() {
            setTimeout(loadGoogleSettings, 500);
        });
    </script>
</body>
</html>
